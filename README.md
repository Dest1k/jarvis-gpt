...
- Reasoning-first понимание задачи: для fuzzy web-запросов агент спрашивает модель (`_understand_intent`), которая понимает интент по смыслу и профилю оператора, а не по ключевым словам; `_looks_like_*`-эвристики остаются детерминированным офлайн-фолбэком. Арбитр может и повышать задачу до миссии: уверенное решение `mission` создаёт persisted mission plan без ключевых слов.
