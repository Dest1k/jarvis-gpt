...
- File ingestion: загрузка текстовых, Word/Excel/PDF/PPTX/ODT/RTF файлов, хранение в `D:\jarvis\data\jarvis-gpt\files`, document extraction, chunk search и audit trail.
- Document surfer: изолированный black-box обработчик документов (`document_surfer`, аналог `web_surfer`) — durable recall из файловой памяти, inspect/read/analyze/compare/search/corpus/generate/convert для Word/Excel/PDF/PPTX/текста, copy-on-write правки и генерация md/docx/xlsx без перезаписи оригиналов.
...
The launcher auto-rebuilds the production frontend when `frontend/app`, `public`, config or lock files are newer than `.next/BUILD_ID`. `jarvis.cmd app` starts the bridge, backend, and UI without starting or later stopping the LLM dispatcher. Full-stack start first reuses an already running dispatcher/OpenAI-compatible endpoint; only when no LLM is active does it start Docker Desktop and the dispatcher. Command Center requires the configured `JARVIS_API_TOKEN` at `/login`; the credential is exchanged for an 8-hour, signed HttpOnly browser session and is not persisted in browser storage or cookies. Set an optional high-entropy `JARVIS_UI_SESSION_SECRET` as supplemental UI-session signing material. If TLS terminates at a trusted reverse proxy, set `JARVIS_TRUST_PROXY_HEADERS=1` only when that proxy overwrites all forwarded headers; this lets Jarvis set `Secure` cookies from the original HTTPS scheme. To expose only the authenticated UI to the local subnet, run `.\jarvis.cmd app -Lan -LanSubnet 192.168.31.0/24`; the backend, dispatcher, and host bridge remain loopback-only, and the UI rejects clients outside the configured subnet. LAN mode persists across launcher restarts; pass `-LocalOnly` to return to localhost-only mode. Use `-NoDockerStart` only for manual Docker diagnostics.
...
Doctor считает запущенные backend, autonomy API и frontend обязательными и
завершается с ненулевым кодом при их недоступности или `{"ok": false}` в health.
Для проверки только кода и сборки без поднятого runtime используйте
`.\scripts\doctor.ps1 -SkipHttp`.
...
- Document intelligence surface: uploaded files or local paths can use
  `documents.recall`, `documents.inspect`, `documents.review`, `documents.read`,
  `documents.compare`, `documents.edit.plan`, and
  `documents.apply_replacements` for Word/Excel/PDF and text-like files.
  `documents.recall` resolves earlier uploads by filename and indexed content,
  reads the persisted source, returns bounded passages plus structural analysis,
  and gives the chat model enough evidence for a source-named summary without a
  current attachment.
...
- Native host bridge uses token-authenticated `action.v1` / `native-app-v3`; arbitrary command execution is removed, `/execute` returns `410 Gone`, and process launch is restricted to a fixed desktop-app/argument grammar.
...
- Browser tools accept only a Chrome CDP endpoint attested to the bridge launch nonce, listening PID/start time, dedicated profile, debug URL, and full guarded command line. A reachable manual or pre-existing CDP endpoint is reported as unattested and is never used for navigation.
...
- `browser.open`, `browser.open_many`, and `browser.chrome.launch` route Chrome through a loopback fail-closed proxy. Every HTTP/CONNECT destination is resolved and checked again, private/link-local/metadata and mixed DNS answers are blocked, and the upstream socket uses the checked numeric IP to prevent a second DNS lookup. Public-only and exact private-host sessions are isolated; explicit localhost policy remains available without allowing a public-to-local redirect pivot.
