...
- Интернет-кейсы закрываются специализированными безопасными маршрутами поверх общего fetch/render-стека: Wayback-архив для заблокированных/исчезнувших страниц (`web.archive`, с пометкой историчности данных), RSS/Atom вместо скрейпинга (`web.feed`), бесключевой геокодированный прогноз вместо погодных сниппетов (`web.weather`, погодный fast-path пробует его первым с честным фолбэком на поиск) и долгоживущий мониторинг страниц (`web.watch` job kind + `web.watch.add/list/remove`): изменение наблюдаемого контента поднимает событие и durable-память, а лимиты (12 активных вотчей, бюджеты job'ов, дедуп) держат автономию ограниченной.
