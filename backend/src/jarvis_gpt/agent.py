...
MISSION_EXECUTOR_PROMPT = (
    ...
    "compare/edit/edit.plan/search/corpus.summarize/generate/convert/file.identify/"
    "file.probe/archive.list/archive.extract/archive.read_member/archive.search/"
    "archive.create. Чтобы ИЗМЕНИТЬ существующий xlsx/docx/текст (добавить строку или "
    "раздел, заменить текст), вызывай documents.edit с file_id и operations — сохранится "
    "новая версия, оригинал не перезаписывается (для точечных замен есть и "
        ...
        lines.extend(
            [
                (
                    ...
                    "documents.recall/inspect/read/analyze/compare/edit/edit.plan/"
                    "apply_replacements/"
